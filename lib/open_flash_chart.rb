...
def _ofc(width, height, url, use_swfobject, base="/", set_wmode_transparent=false)
  ...
  if use_swfobject
    ...
    out << 'var so = new SWFObject("' + base + 'open-flash-chart.swf", "' + obj_id + '","' + width.to_s + '","' + height.to_s + '", "9", "#aaa");'
  ...
  out << '<param name="bgcolor" value="#AAA" />'
	out << '<param name="wmode" value="transparent" />' if set_wmode_transparent
  out << '<embed src="' + base + 'open-flash-chart.swf?data=' + url + '" quality="high" bgcolor="#AAA" width="' + width.to_s + '" height="' + height.to_s + '" name="' + obj_id + '" align="middle" allowScriptAccess="sameDomain" '
